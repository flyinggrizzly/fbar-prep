  ...
  context 'with a complex mapping' do
    ...
    describe 'verifying mappings and data computations' do
      ...
      it 'handles composed first not null and multiplication' do
        expect(transactions.first.amount).to eq(101.01)
        expect(transactions.first.balance).to eq(101.01)

        expect(transactions[1].amount).to eq(-35.10)
        expect(transactions[1].balance).to eq(65.91)
      end
    end
  end
  ...
  context 'with a simple mapping' do
    ...
    context 'with the `:max` strategy' do
      it 'returns the highest balance on the day' do
        expect(account.balance_on(Date.new(2021, 12, 31), :max)).to eq(2100.01)
      end

      it 'returns the latest eod balance on a day without transactions' do
        expect(account.balance_on(Date.new(2022, 01, 01), :max)).to eq(2000.00)
      end
    end

    context 'with the `:eod` strategy' do
      it 'returns the eod balance on the same date' do
        expect(account.balance_on(Date.new(2021, 12, 31), :eod)).to eq(2000.00)
      end

      it 'returns the previous eod balance on following dates' do
        expect(account.balance_on(Date.new(2022, 01, 01), :eod)).to eq(2000.00)
      end
    end
  end
end
